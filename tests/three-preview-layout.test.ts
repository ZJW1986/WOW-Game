...
describe("three preview layout", () => {
  it("keeps renderer, camera, and DOM aspect ratios synchronized", () => {
    ...
    expect(source).toContain('import { createThreeGenreRuntime } from "../runtime/three/genres"');
    ...
    expect(source).toContain('import { createThreePhysicsWorld } from "../runtime/three/Physics"');
    expect(source).toContain("const physics = await createThreePhysicsWorld");
    expect(source).toContain("physics.step(delta)");
    expect(source).toContain('event.type === "collision-start"');
    expect(source).not.toContain("distance2D(player.position, hazard.position) < 0.85");
